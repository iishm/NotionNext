...
async function convertNotionToSiteData(pageId, from, pageRecordMap) {
  ...
  for (let i = 0; i < pageIds.length; i++) {
    ...
    if (properties) {
      if (properties.slug=="photos") {
        // console.log(`bab${JSON.stringify(value.properties)}`);
        const carouselPropId = Object.entries(schema).find(([key, meta]) => meta.name === 'carousel')?.[0]
        const propArr = value.properties[carouselPropId] || []
        // console.log(`properties.id=${properties.id}`);
        attachments = propArr
          // 先筛出那些第二项是数组的行
          .filter(seg => Array.isArray(seg[1]))
          // 只要第一对注解标签是 "a"
          .filter(seg => seg[1][0][0] === 'a')
          // 拿出注解值，即 "attachment:…:filename.jpg"
          .map(seg => seg[1][0][1])
          .map(url => 'https://www.notion.so/image/' +
            encodeURIComponent(url) +
            '?id='+properties.id+'&table=block')
        // console.log(`bab${attachments}`);
      }
  ...
  // 站点基础信息
  let siteInfo = getSiteInfo({ collection, block, NOTION_CONFIG })
  siteInfo.attachments = attachments
